...
module.exports = {
    ...
    async execute(interaction) {
        if (talkedRecently.includes(interaction.user.id)) {
            interaction.reply({ content: `Alou! Stop it, chill outaj. Znaci P L Z, sacekaj tvojih 12 sati pre nego sto ponovo komandu pokrenes.`})
        } else {
            const questions = [
                'Kako se zoves:',
                'Koliko godina imas:',
            ...
            interaction.user.createDM().then(async (channel) => {
                ...
                interaction.user.send(questions[counter++]).catch(() => {
                    return
                })
                ...
                collector.on('end', (collected) => {
                    if (collected.size < questions.length) {
                        interaction.user.send('Nisi odgovorio na vreme').catch(() => {
                            return
                        })
                    }

                    interaction.user.send("Hvala sto si se applyovao za admina, vikkivuk ce uskoro da ti pregleda application.").catch(() => {
                        return
                    })
